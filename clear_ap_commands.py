...
def grab_ap_name(output):
    '''
    Grab only AP name (used for very long ap names)
    :param output:
    :return:
    '''
    preamble = True
...
def clear_ap(ap_list):
    ...
    for ap in ap_list:
        command = 'clear ap config {}'.format(ap)
        commands.append(command)
    return commands
